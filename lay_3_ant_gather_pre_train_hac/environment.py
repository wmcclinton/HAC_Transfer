...
class Environment():
    ...
    def __init__(self, model_name, goal_space_train, goal_space_test, project_state_to_end_goal, end_goal_thresholds, initial_state_space, subgoal_bounds, project_state_to_subgoal, subgoal_thresholds, max_actions = 1200, num_frames_skip = 10, show = False):
        ...
        self.n_apples = 8
        self.n_bombs = 8
        self.activity_range = 6
        self.robot_object_spacing = 2
        self.catch_range = 1
        self.n_bins = 10
        self.sensor_range = 6
        self.sensor_span = math.pi
        self.coef_inner_rew = 0
        self.dying_cost = -5
        self.objects = []
        self.tot_reward = 0
        self.grand_tot_reward = 0
        self.is_done = 0
    ...
    # Get sensor reading for gather task
    def get_readings(self):  # equivalent to get_current_maze_obs in maze_env.py
        ...
        robot_x, robot_y = self.sim.data.qpos[:2]
        ...
        for ox, oy, typ in sorted_objects:
            ...
            half_span = self.sensor_span * 0.5 #replaced with 1 to make full
    ...
    def reset_sim(self, next_goal = None):
        ...
        return np.concatenate([self.get_state(), apple_readings, bomb_readings])
    ...
    def execute_action(self, action):
        ...
        #if done:
            #self.tot_reward = self.dying_cost + self.tot_reward
            #self.is_done = done
            #return self.get_state()  # give a -10 rew if the robot dies
        com = self.sim.data.qpos
        x, y = com[:2]
        reward = 0
        new_objs = []
        #input()
        ...
        self.grand_tot_reward = reward + self.grand_tot_reward
        self.is_done = done
        apple_readings, bomb_readings = self.get_readings()
        return np.concatenate([self.get_state(), apple_readings, bomb_readings])
    ...
    def get_next_goal(self,test):
        ...
        if self.name == "ur5.xml":
            ...
            for i in range(len(self.goal_space_train)):
                ...
                #end_goal[i] = (self.goal_space_train[i][0] + self.goal_space_train[i][1])/2
                end_goal[i] = (self.goal_space_train[i][0])/2
        else:
            ...
            for i in range(len(self.goal_space_test)):
                ...
                #end_goal[i] = (self.goal_space_test[i][0] + self.goal_space_test[i][1])/2
                end_goal[i] = (self.goal_space_test[i][0])/2
