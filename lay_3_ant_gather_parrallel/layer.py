...
class Layer():
    ...
    def choose_action(self,agent, env, subgoal_test):
        if self.layer_number != agent.FLAGS.layers - 1:
            self.current_state = self.current_state[:self.actor.state_dim]
    ...
    def perform_action_replay(self, env, agent, hindsight_action, next_state, goal_status):
        if self.layer_number != agent.FLAGS.layers - 1:
            self.current_state = self.current_state[:self.actor.state_dim]
    ...
    def train(self, agent, env, subgoal_test = False, episode_num = None):
        ...
        while True:
            ...
            if not agent.FLAGS.test:
                ...
                self.perform_action_replay(env, agent, hindsight_action, agent.current_state, goal_status)
            ...
            if (max_lay_achieved is not None and max_lay_achieved >= self.layer_number) or agent.steps_taken >= env.max_actions or attempts_made >= self.time_limit:
                ...
                if not agent.FLAGS.test:
                    ...
                    #print("lets_GETTHISDONE")
                    #print([self.replay_buffer.experiences[i][2] for i in range(len(self.replay_buffer.experiences))])
                    #print(self.layer_number)
                    #for i in range(len(self.replay_buffer.experiences)):
                        #print(self.replay_buffer.experiences[i][4])
                        #print(env.project_state_to_end_goal(env.sim, self.replay_buffer.experiences[i][3]))
                        #print(self.replay_buffer.experiences[i][2])
                        #if self.layer_number == agent.FLAGS.layers - 1:
                            #print(- np.linalg.norm(env.project_state_to_end_goal(env.sim, self.replay_buffer.experiences[i][3])-self.replay_buffer.experiences[i][4]))
                        #else:
                            #print("Not TOP")
    ...
    def learn(self, num_updates):
        ...
        for _ in range(num_updates):
            ...
            if self.replay_buffer.size > 250:
                ...
                old_states = [x[:self.actor.state_dim] for x in old_states]
                new_states = [x[:self.actor.state_dim] for x in new_states]
