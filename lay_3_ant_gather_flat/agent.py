...
class Agent():
    ...
    def initialize_networks(self):
        ...
        if self.FLAGS.retrain == False:
            print("Loaded pre_trained HAC")
            print(lower_hac_model_vars)
            #vars = tf.trainable_variables()
            #print(vars) #some infos about variables...
            #vars_vals = self.sess.run(vars)
            #for var, val in zip(vars, vars_vals):
                #if not (not ("critic_" + str(self.FLAGS.layers - 1) in var.name)) and (not ("actor_" + str(self.FLAGS.layers - 1) in var.name)):
                    #print("var: {}, value: {}".format(var.name, val))
            #quit()

            lower_hac_saver.restore(self.sess, tf.train.latest_checkpoint(self.model_dir))
    ...
    def learn(self):
        ...
        self.layers[self.FLAGS.layers - 1].learn(self.num_updates)
    ...
    def train(self,env, episode_num):
        ...
        return goal_status[self.FLAGS.layers-1], env.grand_tot_reward
